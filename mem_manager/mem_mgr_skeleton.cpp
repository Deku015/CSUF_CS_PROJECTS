...
void fifo_replace_page(size_t& frame) {
    static size_t fifo_index = 0;

    size_t oldest_entry = tlb[fifo_index].npage;
    tlb_remove(fifo_index);

    int frame_index = find_frame_ptable(oldest_entry);
    assert(frame_index != -1);

    frame = pg_table[frame_index].frame_num;

    tlb_add(fifo_index, pg_table[oldest_entry]);
    fifo_index = (fifo_index + 1) % TLB_SIZE;
}

void lru_replace_page(size_t& frame) {

    size_t lru_index = get_used_ptable();

    int frame_index = find_frame_ptable(lru_index);
    assert(frame_index != -1);

    frame = pg_table[frame_index].frame_num;

    int tlb_index = check_tlb(lru_index);
    if (tlb_index != -1) {
...
void check_address_value(size_t logic_add, size_t page, size_t offset, size_t physical_add,
    ...
    printf("log: %5lu 0x%04x (pg:%3lu, off:%3lu)-->phy: %5lu (frm: %3lu) (prv: %3lu)--> val: %4d == value: %4d -- %s", 
          (unsigned long)logic_add, (unsigned int)logic_add, page, offset, physical_add, frame, prev_frame, 
...
void run_simulation() { 
    ...
    size_t min_page_in_memory;
    size_t max_page_in_memory;
    bool loadPagesIfNotPresent(size_t page){
        if(page < min_page_in_memory || page > max_page_in_memory){
